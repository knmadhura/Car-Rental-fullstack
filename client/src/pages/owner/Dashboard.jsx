...
const fetchDashboardData = async () => {
  try {
    ...
    console.log("📦 DashboardData response:", res.data); // 🔍 Debug log
    ...
    if (res.data.success && res.data.dashboardData) {
      setData({
        ...
        pendingBookings: res.data.dashboardData.pendingBookings || 0,
         confirmedBookings: res.data.dashboardData.confirmedBookings || 0,
  ...
  // 🛡 Optional defensive render (optional but safe)
      ...
      <div className='flex flex-wrap items-start gap-6 mb-8 w-full'>
        ...
        <div className='p-4 md:p-6 border border-borderColor rounded-md max-w-lg w-full'>
          ...
          {data.recentBookings.length === 0 ? (
            <p className="mt-4 text-sm text-gray-400">No recent bookings</p>
          ) : (
              ...
              <div key={index} className='mt-4 flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                  ...
                  <div>
                    ...
                    <p className='text-sm text-gray-500'>{booking.createdAt?.split('T')[0] || 'Unknown Date'}</p>
                  </div>
                </div>
